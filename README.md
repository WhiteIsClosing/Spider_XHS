...
### 🛡️ 反爬 / 防风控（单 IP + 单 Cookie 必读）

如果你**没有代理池、只手动提供一个 Cookie**，请先读 **[反爬对抗使用指南 → ANTI_CRAWL.md](./ANTI_CRAWL.md)**。
里面写清楚了：你该做什么、运行时如何看 `[风控]` / `[限速]` / `[Budget]` 日志、命中熔断/配额/签名过期时怎么办，以及全部可调环境变量（退避、熔断阈值、昼夜节律、单日配额等）。

核心一句话：**单 IP 下速度就是风险**——保持默认的保守节奏，命中风控会自动退避/熔断，别去抢速度、别熔断后立刻重跑。
